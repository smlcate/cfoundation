      ...
      .then(function(res) {
        ...
        for (var i = 0; i < options.length; i++) {
          for (var j = 1; j < options[i].shippingRate_data.rates[0].length-1; j++) {
            ...
            var sep = options[i].shippingRate_data.rates[0][j].split('-');
            if (sep.length == 2) {
              sep = [Number(sep[0]), Number(sep[1].split(' ')[0])];
            } else {
  ...
  function pickCities() {
    ...
    for (var k = 0; k < cities.length; k++) {
      if (cities[k].state == $scope.order.shipping.state){
        $scope.citiesToShow.push(cities[k]);
      }
    }

    var html = '<option value="{{c.city}}" ng-repeat="c in citiesToShow track by $index">{{c.city}}</option></select>';
    angular.element($('#shippingCityDropdown')).append($compile(html)($scope));

    if ($scope.citiesToShow.length != 0) {
      $('#shippingCityDropdown').css({
        borderColor: '#C4B0FF',
        pointerEvents: 'all',
        cursor: 'pointer'
      })
